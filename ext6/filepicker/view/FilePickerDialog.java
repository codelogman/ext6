...
package tar.eof.ext6.filepicker.view;
...
import tar.eof.ext6.filepicker.controller.NotifyItemChecked;
import tar.eof.ext6.filepicker.controller.adapters.FileListAdapter;
import tar.eof.ext6.filepicker.model.DialogConfigs;
import tar.eof.ext6.filepicker.model.DialogProperties;
import tar.eof.ext6.filepicker.model.FileListItem;
import tar.eof.ext6.filepicker.model.MarkedItemList;
import tar.eof.ext6.filepicker.utils.ExtensionFilter;
import tar.eof.ext6.filepicker.utils.Utility;
...
public class FilePickerDialog extends Dialog implements AdapterView.OnItemClickListener {
    ...
    private TextView dname, dir_path, title, txtdelete;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        alsodelete = (MaterialCheckbox) findViewById(R.id.alsodeleteall);
        txtdelete = (TextView) findViewById(R.id.txtdeleteafter);
        int size = MarkedItemList.getFileCount();
        if (size == 0) {
            alsodelete.setEnabled(false);
            select.setEnabled(false);
            int color;
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                color = context.getResources().getColor(R.color.control_highlight, context.getTheme());
            } else {
                color = context.getResources().getColor(R.color.control_highlight);
            }
            select.setTextColor(Color.argb(128, Color.red(color), Color.green(color), Color.blue(color)));
            txtdelete.setTextColor(Color.argb(128, Color.red(color), Color.green(color), Color.blue(color)));
        }
		...
		alsodelete.setOnCheckedChangedListener(new OnCheckedChangeListener() {
			@Override
        ...
        select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (callbacks != null) {
                    callbacks.onSelectedFilePaths(paths, alsodelete.isChecked());
                }
                dismiss();
            }
        });
        ...
        if(properties.alsodelete == false){
            alsodelete.setVisibility(View.GONE);
        ...
        mFileListAdapter.setNotifyItemCheckedListener(new NotifyItemChecked() {
            ...
            public void notifyCheckBoxIsClicked() {
                ...
                if (size == 0) {
                    ...
                    txtdelete.setTextColor(context.getResources().getColor(R.color.txt_delete));
